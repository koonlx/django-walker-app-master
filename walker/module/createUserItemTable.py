...
@logging_time
def create_stores_table(user_num, curr_lat=35.8620675, curr_lon=129.1922453, km=2) -> DataFrame:
    """
    ...
    # restaurants = RestaurantInfo.objects.filter(위도__gt=curr_lat - 2/109.958489129649955, 위도__lt=curr_lat + 2/109.958489129649955, 경도__gt=curr_lon - 2/88.74, 경도__lt=curr_lon + 2/88.74)
    ...
    # df = pd.DataFrame(list(RestaurantInfo.objects.all().values()))
    df = pd.DataFrame(list(RestaurantInfo.objects.filter(위도__gt=curr_lat - 2/109.958489129649955, 위도__lt=curr_lat + 2/109.958489129649955, 경도__gt=curr_lon - 2/88.74, 경도__lt=curr_lon + 2/88.74).values()))
...
def distance_cal(df: DataFrame, lat, lon, km):  # 강남역 10번출구 위도, 경도
    ...
    results = df[df['distance'] < km]
    print(results.sort_values('distance'))
    return df 
